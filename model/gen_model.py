...
class Seq2SeqModel(object):
    ...
    def __init__(self, config, name_scope, forward_only=False, num_samples=256, dtype=tf.float32):
        ...
        for i in xrange(self.buckets[-1][0]):  # Last bucket is the biggest one.
            ...
            # the size of self.encoder_inputs is the biggest query length in bucket
        for i in xrange(self.buckets[-1][1] + 1):
            self.decoder_inputs.append(tf.placeholder(tf.int32, shape=[None], name="decoder{0}".format(i)))
            self.target_weights.append(tf.placeholder(dtype, shape=[None], name="weight{0}".format(i)))
            # the size of self.decoder_inputs and self.target_weights is the biggest response length in bucket
        self.reward = [tf.placeholder(tf.float32, name="reward_%i" % i) for i in range(len(self.buckets))]
        # the size of self.reward is the size of buckets

        # Our targets are decoder inputs shifted by one.
        targets = [self.decoder_inputs[i + 1] for i in xrange(len(self.decoder_inputs) - 1)]
        # the size of targets is the biggest response length - 1
        ...
        self.outputs, self.losses, self.encoder_state = rl_seq2seq.model_with_buckets(
            ...
            lambda x, y: seq2seq_f(x, y, tf.where(self.forward_only, True, False)),
            output_projection=output_projection)
        # 1.self.outputs: two dimension list, 2nd dimension has the size of buckets,1nd dimension has the size of decoder
        # steps,each element is [batch_size,hidden_dim]
        # 2.self.losses: list of size buckets,each element is the loss for current bucket
        # 3.self.encoder_state:list of size buckets,each element is the last hidden state of encoder layer for current bucket
        ...
        self.saver = tf.train.Saver(self.gen_variables,max_to_keep=100)

    def step(self, session, encoder_inputs, decoder_inputs, target_weights,
             bucket_id, forward_only=True, reward=1, mc_search=False, up_reward=False, debug=True, update_gradient=True):
        ...
        # self.outputs: two dimension list, 2nd dimension has the size of buckets,1nd dimension has the size of decoder
        # steps,each element is [batch_size,target_vocab_size]

        return_outputs = session.run(output_feed, input_feed)
        if not forward_only:
            if update_gradient:
                return return_outputs[1], return_outputs[2], return_outputs[0]  # Gradient norm, loss, no outputs.
            else:
                return return_outputs[0]
        else:
            return return_outputs[0], return_outputs[1], return_outputs[2:]  # encoder_state, loss, outputs.
    ...
    def get_batch(self, train_data, bucket_id, batch_size, type=0):
        ...
        for batch_i in xrange(batch_size):
            if type == 1:
                ...
                #we random choose a pair of training data from the give bucket_id list
            ...
            #the length of encoder_inputs will be pad to the length of bucket query length
            #but strangely the pad token will be at the front of the encoder_input

            # Decoder inputs get an extra "GO" symbol, and are padded then.
            decoder_pad_size = decoder_size - len(decoder_input) - 1
            decoder_inputs.append([util.GO_ID] + decoder_input +
                                  [util.PAD_ID] * decoder_pad_size)
            #the length of decoder_input will be pad to the length of bucket response length
            #the first token will be GO_ID

            # print(str(list(reversed(encoder_input + encoder_pad))))
            # print(str([util.GO_ID] + decoder_input +
            #                     [util.PAD_ID] * decoder_pad_size))
            # assert 0 == 1
        ...
        # batch_decoder_inputs will be two dimension lists, 2r dimension is of size current_sequence_length,
        # 1r dimension is of size batch_size

        # batch_weights will be two dimension lists, 2r dimension is of size current_sequence_length,
